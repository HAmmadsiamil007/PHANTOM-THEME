...
function acf_blocks_init() {
  ...
  $example_data = apply_filters( 'optix_acf_blocks_example_data', [] );
  ...
  foreach ( THEME_SETTINGS['acf_blocks'] as $block ) {
    ...
    if ( ! isset( $block['icon'] ) || empty( $block['icon'] ) ) {
      ...
      $icon_path = apply_filters( 'optix_acf_block_icon', $icon_path, $block['name'], $block );
    ...
    // Use ACF PRO if available, otherwise fall back to native WP block registration
    if ( function_exists( 'acf_register_block_type' ) ) {
      acf_register_block_type( wp_parse_args( $block, THEME_SETTINGS['acf_block_defaults'] ) );
    } else {
      register_block_type( 'acf/' . $block['name'], wp_parse_args( $block, THEME_SETTINGS['acf_block_defaults'] ) );
    }
  }
} // end acf_blocks_init
