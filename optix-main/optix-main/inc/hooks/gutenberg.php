...
/**
 * Get asset file path (replaces Air-light's build system path).
 * Since this theme uses static assets directly, returns path relative to theme.
 */
function get_asset_file( $file ) {
  $path = 'dist/' . $file;
  return file_exists( get_theme_file_path( $path ) ) ? $path : '';
}
...
function allowed_block_types( $allowed_blocks, $editor_context ) { // phpcs:ignore
  ...
  if ( $select_all || $core_blocks ) {
    ...
    $core_blocks = apply_filters( 'optix_allowed_core_blocks_obj', $core_blocks );
    ...
    $core_blocks = apply_filters( 'optix_allowed_core_blocks', $core_blocks );
...
function register_block_editor_assets() {
  $editor_js = get_asset_file( 'editor.js' );
  if ( ! $editor_js ) {
    return;
  }
  ...
  $dependencies = [
    ...
    'wp-components', // Provides many prebuilt components and extensions
  ];

  // Enqueue the bundled block JS file
  wp_enqueue_script(
    'block-editor-js',
    get_theme_file_uri( $editor_js ),
    $dependencies,
    filemtime( get_theme_file_path( $editor_js ) ),
    'all'
  );
...
function setup_editor_styles() {
  ...
  $editor_css = get_asset_file( 'editor.css' );
  if ( $editor_css ) {
...
function block_editor_title_input_styles() {
  ...
  body.locale-fi .editor-styles-wrapper .editor-post-title::before {
    content: "Nimi, joka nÃ¤kyy selaimen vÃ¤lilehdessÃ¤ ja valikossa";
  }
