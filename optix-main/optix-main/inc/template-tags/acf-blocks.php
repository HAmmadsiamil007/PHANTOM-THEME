...
function render_acf_block( $block, $content = '', $is_preview = false, $post_id = 0 ) {
  ...
  $cache_key    = apply_filters( 'optix_acf_block_cache_key', $cache_key, $block_slug, $post_id );

  global $optix_current_block;
  $optix_current_block = $block;
...
function load_acf_block_from_cache( $cache_key, $block_slug, $block_path, $block, $is_preview = false, $post_id = 0 ) {
  ...
  // Save block to cache
  \wp_cache_set( $cache_key, $output, 'theme', apply_filters( 'optix_acf_block_cache_lifetime', HOUR_IN_SECONDS, $block_slug, $post_id ) );
...
function acf_block_maybe_enable_cache( string $block_slug ) {
  ...
  if ( empty( THEME_SETTINGS ) || empty( THEME_SETTINGS['acf_blocks'] ) ) {
    ...
    return apply_filters( 'optix_acf_block_maybe_enable_cache', $enable_cache, null );
  }
  ...
  if ( false === $block_key ) {
    ...
    return apply_filters( 'optix_acf_block_maybe_enable_cache', $enable_cache, $block_slug );
  }
